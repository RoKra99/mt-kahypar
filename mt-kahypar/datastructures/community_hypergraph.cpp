...
CommunityHypergraph CommunityHypergraph::contract(StaticHypergraph& hypergraph, parallel::scalable_vector<HypernodeID>& communities) {
    ...
    hypergraph = _hg->contract(communities, 0, !_use_multipins);
    //utils::Timer::instance().start_timer("community_specific", "Community Specific");
    CommunityHypergraph chg(_context, _use_multipins);
    chg._hg = &hypergraph;
    chg._vol_v = _vol_v;
    ...
    // if multipins are in use build a new incidence array for them
    if (_use_multipins) {
    ...
    // construct new community count datastructure
    chg._community_counts.resize(chg.initialNumEdges());
    chg.computeAndSetCommunityCounts();
